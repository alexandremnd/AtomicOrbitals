...
TEST(OverlapIntegralTest, SlaterPrimitiveValue) {
    ...
    EXPECT_NEAR(overlap_integral(orbital1, orbital2) / orbital1.normalization() / orbital2.normalization(), 11.76047764474325, 1e-5);

    orbital1.set_n(3); orbital2.set_n(8);
    EXPECT_NEAR(overlap_integral(orbital1, orbital2) / orbital1.normalization() / orbital2.normalization(), 34505.28801422153, 1e-3);
}
