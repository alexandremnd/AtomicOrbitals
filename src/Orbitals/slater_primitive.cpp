...
double electron_electron_integral(const SlaterPrimitive &orbital1,
    ...
    int max_L =
        std::min({orbital1.l() + orbital2.l(), orbital3.l() + orbital4.l()});
    ...
    for (int L = 0; L <= max_L; L++) {
        ...
        for (int M = -L; M <= L; M++) {
            ...
            if (-orbital1.m() + orbital2.m() + M != 0 ||
                -orbital3.m() + orbital4.m() - M != 0) {
                continue;
            }

            double m_value = (M % 2 == 0) ? 1. : -1.;
            m_value *= angular_integral(orbital1, orbital2, L, M);
            m_value *= angular_integral(orbital3, orbital4, L, -M);
