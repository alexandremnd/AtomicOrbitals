...
inline double wigner_3j(int j1, int j2, int j3, int m1, int m2, int m3) {
    ...
    double term =
        1. / boost::math::tgamma(K + 1) / boost::math::tgamma(t1 - K + 1) /
        boost::math::tgamma(t2 - K + 1) / boost::math::tgamma(t3 - K + 1) /
        boost::math::tgamma(t4 + K + 1) / boost::math::tgamma(t5 + K + 1);
    sum *= term;
    ...
    double prefactor =
        phase_factor * std::sqrt(boost::math::tgamma(j1 + j2 - j3 + 1) *
